      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-block px-4 py-1.5 bg-[#FFF7ED] border border-[#FED7AA] rounded-full mb-4">
            <span className="text-sm text-[#ed6b1d] font-medium">Servicios</span>
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service, index) => (
            ...
            >
              <div className="w-12 h-12 bg-[#FFF7ED] group-hover:bg-[#ed6b1d] rounded-xl flex items-center justify-center mb-4 transition-colors">
                <service.icon className="w-6 h-6 text-[#ed6b1d] group-hover:text-white transition-colors" />
              </div>
              ...
              <ul className="space-y-2">
                {service.bullets.map((bullet, idx) => (
                  <li key={idx} className="flex items-start gap-2 text-sm text-[#64748B]">
                    <span className="w-1 h-1 bg-[#ed6b1d] rounded-full mt-2 flex-shrink-0"></span>
                    <span>{bullet}</span>
                  </li>
                ))}
              </ul>
